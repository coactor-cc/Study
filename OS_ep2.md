# 进程与线程
## 预备——管理和使用CPU
操作系统在管理CPU的时候搞出来了**多进程pattern**，有了多进程pattern问题才得以推进
...
patten：启动多个进程，CPU的运行的样子

Linux从开机到结束： if(!fork()){init();}   
操作系统开机最后启动了一个shell   
shell不断等待命令，来了就创建进程
...
### 概念
通俗：执行中的程序  
是进程实体`程序`的运行过程，是系统进行资源**分配**和**调度***若内核支持线程，则线程*的独立单位  

进程包括PCB【机器状态信息PC、IR、PSR，现场信息GPR】+内存区域【堆栈段，数据段】
### 进程状态
熟稔于心
### 进程控制块PCB
进程状态标记  
...
**解1**  
最多允许4个哲学家吃饭  
评价：这是一种基于具体情景设计的解决方案，并不具有普适性，只能在n个进程都只请求2种资源，且满足单环互斥的情景
